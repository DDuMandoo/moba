def analyze_group_interests(tokens: list):
    ...
    for token in tokens:
        try:
            ...
            print(f"{token}의 user_id : {user_id}")
            valid_user_ids.append(user_id)

            result = get_latest_recommendation(user_id)
            print(f"🟢 유저 {user_id}의 추천 : {result}")
            if result:
    ...
    # 점수 집계
    category_sub_score_sum = {cat: {} for cat in category_to_subcategories}
    subcategory_count = {cat: {} for cat in category_to_subcategories}
